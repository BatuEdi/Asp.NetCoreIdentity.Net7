...
builder.Services.ConfigureApplicationCookie(opt =>
{
    ...
    opt.LogoutPath = new PathString("/Home/logout");  // burada yapýlan çýkýþ yap iþlemini farklý yerlere yönlendrimek istediðimiz zaman kullanýyoruz. Member Controller da çýkýþ yap iþlemi yapýldýktan sonra hangi sayfaya yönlendireleceði _NavbarLogin de ki asp-rout-routurel="" ile belirtilir.
    opt.Cookie = cookieBuilder;
    opt.ExpireTimeSpan = TimeSpan.FromDays(60);
    opt.SlidingExpiration = true;    // bu özelllik kullanýcý 60 gün içinde bir kez daha giriþ yaparsa 60 gün süresi yenilenir.
...
using (var scope = app.Services.CreateScope()) //Burada sýfýrdan bir scope oluþturmamýzýn nedeni uygulamanýn bir kere çalýþtýrýlýp memory'den düþmesi için
{
